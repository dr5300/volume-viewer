...
export class Material {
	...
	resetProgramFlagsAndValues(){
		...
		if (this._maps.length > 0) {
			...
			// should we check each texture in _maps array???
			if(this._maps[0].constructor.name === "Texture") {
                this._flags.push("TEXTURE");
			} else if (this._maps[0].constructor.name === "Texture3D") {
                this._flags.push("TEXTURE3D");
			}
