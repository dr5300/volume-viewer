...
export class MeshRenderer extends Renderer {
		...
		for (let i = 0; i < attributes.length; i++) {
			...
			switch (attributes[i]) {
				...
				case "uv":
					...
					break;
                case "uvw":
                    let uvw = object.geometry.uvw;
                    buffer = this._glManager.getAttributeBuffer(uvw);
                    attributeSetter["uvw"].set(buffer, 3, object.instanced, uvw.divisor);
                    break;
				case "MMat":
	...
	_setup_material_uniforms(material, uniformSetter) {//TODO: Move to object specific - same as draw (Sebastien)
		...
		if (uniformSetter["material.specular"] !== undefined) {
            const specular = material.specular ? material.specular : material.getUniform("material.specular");
			uniformSetter["material.specular"].set(specular.toArray());
		}
		if (uniformSetter["material.shininess"] !== undefined) {
			const shininess = material.shininess ? material.shininess : material.getUniform("material.shininess");
			uniformSetter["material.shininess"].set(shininess);
		}
		//}
